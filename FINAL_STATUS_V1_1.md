...
后续用户 Edge 截图确认旧版在线底图和已规划路线可见。本轮完成最终 UI 排版与地图性能修正：4+3指标、窄卡换行、路线聚焦、选点状态、紧凑状态栏和560px地图。浏览器连接仍失败，修复后三种宽度和真实鼠标操作尚未人工验收，保持 `pending`。
...
- 原有 **78 项测试及断言全部保留**；本轮新增16项性能架构测试，最终 **94 项测试全部通过**（39.947秒）。日志：`work/ui_final_performance_tests.log`。

## 本轮 UI/runtime 性能修正

- 服务器端地图组件处理（含选点吸附）：在线约2.6秒→6毫秒，离线约2.6–2.7秒→125–129毫秒；包含风险状态/寻路的一次规划约148–267毫秒。不是浏览器端到端延迟。
- 完整 HTML 体积：在线规划图约13.42 MB→25.6 KB，离线约13.42 MB→662.5 KB。正式55,727条候选路网不变；新增586,930 B的显示专用简化几何。
- 固定地图key与基础脚本，起终点/路线增量覆盖；只有新路线才fit bounds，普通选点保留视角。原始/吸附位置和节点详情可折叠查看。
...
测量记录：`results/ui_performance_before.json`、`results/ui_performance_after.json`。详情见 [UI性能报告](docs/UI_PERFORMANCE_REPORT.md)；浏览器绘制/瓦片延迟未测得，所有人工勾选项保持未勾选。
